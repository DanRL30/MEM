...
class TestTransiciones:
    def test_el_camino_completo(self) -> None:
        assert (
    ...
    def test_editar_devuelve_a_borrador(self) -> None:
        assert (
            verificar(Estado.CALCULADA, Accion.EDITAR_INSUMOS, Perfil.LIDER_DE_ESTUDIO)
            is Estado.BORRADOR
        )

    def test_congelar_es_irreversible(self) -> None:
    ...
    def test_nada_modifica_una_evaluacion_congelada(self, accion: Accion) -> None:
    ...
    def test_el_mensaje_orienta_hacia_el_recalculo(self) -> None:
    ...
    def test_recalcular_no_altera_la_corrida_congelada(self) -> None:
        assert (
            verificar(Estado.CONGELADA, Accion.RECALCULAR, Perfil.LIDER_DE_ESTUDIO)
            is Estado.CONGELADA
        )

    def test_recalcular_no_aplica_a_una_corrida_no_congelada(self) -> None:
...
class TestPermisos:
    def test_el_ingeniero_ejecuta_pero_no_congela(self) -> None:
        verificar(Estado.BORRADOR, Accion.EJECUTAR, Perfil.INGENIERO_DE_PROYECTO)
    ...
    def test_finanzas_no_congela_evaluaciones_de_proyectos(self) -> None:
    ...
    def test_los_perfiles_de_consulta_no_tienen_acciones(self, perfil: Perfil) -> None:
        for estado in Estado:
            assert acciones_disponibles(estado, perfil) == ()

    @pytest.mark.parametrize("perfil", [Perfil.CONSULTA_EJECUTIVA, Perfil.AUDITOR])
    def test_los_perfiles_de_consulta_no_pueden_recalcular(self, perfil: Perfil) -> None:
        with pytest.raises(PerfilNoAutorizado, match="consulta"):
            verificar(Estado.CONGELADA, Accion.RECALCULAR, perfil)

    def test_el_error_indica_quien_si_puede(self) -> None:
...
class TestBitacora:
    def test_no_expone_modificacion_ni_borrado(self) -> None:
    ...
    def test_encadena_las_entradas(self) -> None:
    ...
    def test_detecta_la_supresion_de_una_entrada(self) -> None:
        b = _bitacora_con_tres_entradas()
        b._entradas.pop(1)
        with pytest.raises(BitacoraCorrupta, match="Cadena rota"):
            b.verificar()

    def test_detecta_la_modificacion_de_una_entrada(self) -> None:
        b = _bitacora_con_tres_entradas()
    ...
    def test_detecta_el_reordenamiento(self) -> None:
        b = _bitacora_con_tres_entradas()
        b._entradas[0], b._entradas[1] = b._entradas[1], b._entradas[0]
        with pytest.raises(BitacoraCorrupta):
            b.verificar()

    def test_exige_identificar_al_responsable(self) -> None:
        b = Bitacora()
    ...
    def test_una_modificacion_debe_registrar_ambos_valores(self) -> None:
        b = Bitacora()
    ...
    def test_registra_el_cambio_con_sus_dos_valores(self) -> None:
        b = Bitacora()
        e = b.registrar(
            "R1",
            "C1",
    ...
    def test_el_historial_de_cambios_omite_lo_que_no_modifica(self) -> None:
        b = _bitacora_con_tres_entradas()
        b.registrar(
            "R1",
            "C1",
...
class TestCoberturaDelAlcance:
    def test_cubre_los_ocho_requisitos_de_gobierno(self) -> None:
        assert sorted(COBERTURA) == list(range(1, 9))

    def test_cada_requisito_apunta_a_una_implementacion(self) -> None:
