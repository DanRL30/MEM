...
def costo_sql(modo: str) -> float:
    ...
    if modo == "serverless_con_pausa":
        ...
        return (
            0.75 * HORAS_LABORALES_MES * 3600 * P["sql_serverless_vcore_s"]
            + 32 * P["sql_almacenamiento_gb"]
        )
...
def escenario_a() -> list[Entorno]:
    ...
    prod.agregar(
        ...
        costo_apim("standardv2"),
        "SLA 99,95 % · integración a red virtual",
    )
    prod.agregar(
        "Cómputo Elastic Premium EP1",
        costo_computo("ep1"),
        "Ranuras de despliegue · sin arranque en frío",
    )
    prod.agregar(
        "Azure SQL aprovisionada 2 vCore",
        costo_sql("aprovisionada_2vcore"),
        "Costo fijo y predecible",
    )
    prod.agregar(
        "Front Door Premium",
        P["frontdoor_premium_mes"],
        "WAF gestionado DRS · Private Link al origen",
    )
    ...
    qa.agregar(
        ...
        costo_apim("developer"),
        "Sin SLA · aceptable fuera de producción",
    )
    ...
    qa.agregar(
        "Front Door Premium",
        P["frontdoor_premium_mes"],
        "Igual a producción para el ethical hacking",
    )
...
def escenario_b() -> list[Entorno]:
    ...
    prod = Entorno("Producción")
    prod.agregar(
        "API Management Basic v2", costo_apim("basicv2"), "Conserva SLA 99,95 % · sin red virtual"
    )
    prod.agregar("Cómputo Elastic Premium EP1", costo_computo("ep1"), "Se conservan las ranuras")
    prod.agregar(
        "Azure SQL serverless sin pausa",
        costo_sql("serverless_sin_pausa"),
        "Sin latencia de reanudación",
    )
    ...
    qa.agregar(
        ...
        costo_apim("consumption"),
        "Pago por llamada · primer millón sin costo",
    )
    ...
    dev.agregar(
        "Puntos de conexión de servicio", 0.0, "Sin costo · menor aislamiento que los privados"
    )
...
def escenario_c() -> list[Entorno]:
    ...
    prod = Entorno("Producción")
    prod.agregar(
        "API Management Consumption", costo_apim("consumption"), "SLA 99,95 % · escala a cero"
    )
    prod.agregar(
        "Cómputo Flex, 1 instancia siempre lista",
        costo_computo("flex_siempre_listo"),
        "Evita el arranque en frío",
    )
    prod.agregar(
        "Azure SQL serverless con pausa y calentamiento", costo_sql("serverless_con_pausa")
    )
    prod.agregar(
        "Front Door Standard",
        P["frontdoor_standard_mes"],
        "WAF con reglas propias · sin DRS gestionado",
    )
    prod.agregar("Puntos de conexión privados (4)", costo_pe(4))
...
def escenario_d() -> list[Entorno]:
    ...
    prod.agregar(
        "Cómputo Elastic Premium EP1",
        costo_computo("ep1"),
        "Se conservan las ranuras de despliegue",
    )
    prod.agregar(
        "Azure SQL serverless con pausa y calentamiento",
        costo_sql("serverless_con_pausa"),
        "Activa de 07 a 18 en días hábiles",
    )
    prod.agregar(
        "Front Door Standard",
        P["frontdoor_standard_mes"],
        "WAF con reglas propias · origen restringido a Front Door",
    )
    prod.agregar("Puntos de conexión privados (4)", costo_pe(4))
    prod.agregar("Observabilidad con tope diario", costo_observabilidad(4))
    prod.agregar("Almacenamiento", costo_almacenamiento())
    ...
    qa.agregar(
        "Front Door Standard",
        P["frontdoor_standard_mes"],
        "Igual a producción: se homologa lo que se despliega",
    )
    qa.agregar("Puntos de conexión privados (4)", costo_pe(4))
    qa.agregar("Observabilidad", costo_observabilidad(2))
    qa.agregar("Almacenamiento", costo_almacenamiento() * 0.6)
...
def main() -> int:
    ...
    print(
        f"  {USUARIOS_NOMINALES} usuarios · {USUARIOS_CONCURRENTES} concurrentes · "
