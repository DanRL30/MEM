...
| 060 | `InputsCapex!7` y `!8`, banda 1 | La puerta que separa capital inicial de sostenimiento **se aplica a una sola unidad**: `IF(fila47=0, NZ_Total-NZ_NOD, 0)`, donde la fila 47 parece la cabecera del bloque de Nazareth y en realidad lleva `IF(InputsProd!94>0,1,0)`, el ejercicio en que Nazareth entrega concentrado. El capital depreciable de las demas unidades es sostenimiento siempre, produzcan o no | La puerta no es una regla de etapa: es el proyecto que el libro evalua, cableado por unidad. La plataforma la generaliza a todas, y contra el caso 1 eso desplaza 466 939,2 k$ -el 23 % del capital del caso- de sostenimiento a inicial: Pisco y B4 no producen nunca por diseno y San Rafael Potencial no produce antes de 2033. **El cierre y el total coinciden al centimo**, que confirma la primera de las tres reglas. Mismo criterio que la 036 y la 039: una excepcion alojada en la formula de una unidad concreta no tiene donde vivir cuando lo que se evalua es un proyecto que hoy no existe. Pendiente del acta | | 02/09/2026 | `capex.py` |
