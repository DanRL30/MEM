...
| 062 | `FC NZ!39`, columna `H` | El factor de descuento del **primer ejercicio del horizonte es un cero tecleado**, no un uno: `H39 = 0` y de la `I` en adelante `1/(1+r)^t`. El NPV de la fila 41 suma desde la `I`, de modo que el primer ejercicio queda fuera dos veces | **El libro descarta el flujo del primer ano al valorar.** Contrastado sobre el caso 7 el 02/09/2026: reproducirlo da 6 398 620,3 al centimo, y el motor, que lo descuenta con factor uno, da 7 234 587,2. La diferencia es exactamente el flujo de 2025. Contradice `DM-STD-PE-27` 5.1 y la regla `005`, y **cambia el indicador de cabecera**: no se implementa sin respuesta de Finanzas | | 02/09/2026 | |
...
| 067 | `Depreciacion!126` frente al motor, caso 7 | Contrastada la via tributaria, el total queda a **96 808,7 k$ de 2 855 001,6, un 3,4 %**, y la diferencia no es de nivel sino de reparto en el tiempo: el libro concentra 382 851,4 de mas en 2037 y devuelve casi lo mismo repartido entre 2038 y 2041. Varios ejercicios coinciden exactos | El pico de 2037 **no sale de las cinco filas visibles** -suman 144 625,2 ese ano- sino del bloque de Santo Domingo que entra por el `+H456` de la regla `065`. Apunta a que ese bloque libera de golpe la depreciacion acumulada antes de producir, donde el motor la reparte desde el primer ejercicio productivo. Falta leer su cronograma fila a fila para cerrarlo | | 02/09/2026 | `depreciacion.py` |
