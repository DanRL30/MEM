...
| 062 | `FC NZ!39`, columna `H` | El factor de descuento del **primer ejercicio del horizonte es un cero tecleado**, no un uno: `H39 = 0` y de la `I` en adelante `1/(1+r)^t`. El NPV de la fila 41 suma desde la `I`, de modo que el primer ejercicio queda fuera dos veces | **El libro descarta el flujo del primer ano al valorar.** Reproducido al centimo en los **dos** casos contrastados el 02/09/2026: 4 862 348,7 en el 1 y 6 398 620,3 en el 7, y en los dos la diferencia con el motor es exactamente el flujo del primer ejercicio. **No se cancela al comparar casos**: el NPV incremental del proyecto vale 1 536 271,6 con la convencion del libro y 1 495 563,2 con la del motor, un 2,65 % de diferencia sobre el numero que la plataforma existe para producir. Contradice `DM-STD-PE-27` 5.1 y la regla `005`: no se implementa sin respuesta de Finanzas | | 02/09/2026 | |
