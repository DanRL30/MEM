...
| 081 | `Otros!43:55` frente a `corrida._bolsa_de_egresos` | La `Bolsa Egresos` del libro suma once conceptos y el motor sumaba doce: agregaba `Gestion Social`, que el libro no lleva en este bloque. El propio docstring de la funcion enumeraba los once del libro | La gestion social sale entera de caja y el libro la deja fuera de la base de las cuentas por pagar y del IGV de compras. Con el saldo en `base x dias / 360` movia la variacion del capital de trabajo, y ninguna prueba lo acusaba porque ningun caso del arnes declaraba ese gasto. **La plataforma se alinea al modelo**, con el mismo criterio de la `059`: el concepto sale de la bolsa y sigue llegando al flujo operativo por su propia linea | Project Manager | 04/09/2026 | `otros.py` |
| 082 | `Otros!28:34` frente a `corrida.calcular` | El bloque `Otros Gastos/ Ingresos` del libro lleva donaciones, servidumbre, reguladores con el fondo de jubilacion, la fila tecleada `Otros` y la regalia de la regla `080`, y **no lleva la planilla**: el libro la teclea en `Otros!52`, dentro de la bolsa de egresos. El motor la suma a la linea de otros gastos del flujo operativo | O el libro la recoge en otra linea de `FC NZ` o el motor la cuenta dos veces, una en la bolsa y otra en el flujo. La bolsa no vuelve al flujo, de modo que la segunda lectura implicaria un doble cargo. Se reporta antes de tocar el flujo | | | `corrida.py` |
