...
| 062 | `FC NZ!38:39` y `FC escenarios!42:43` | El ano corre `0, 1, 2, ...` desde el primer ejercicio y el factor de descuento es `0` **tecleado** en ese primer ano, `1/(1+r)^t` desde el segundo. Las dos hojas de valoracion usan la misma convencion | **No es un descuido: es el costo hundido.** El eje de exponentes lleva el relleno con que el libro marca sus inputs, el bloque oculto de la misma hoja usa ese control once columnas mas a la derecha y `Resumen!92` nombra el periodo con una linea `Costo Hundido (2017-25)`. Coincide con `DM-STD-PE-27` §5, que fecha la evaluacion en el sancionamiento y deja fuera del flujo descontado todo lo anterior. **Se reproduce desde el 04/09/2026**: el caso declara su ultimo ejercicio hundido y esos ejercicios llevan factor cero, con lo que el NPV cierra con brecha 0,00 contra la fila 41. Lo decide el [ADR 0012](../adr/0012-el-costo-hundido-sale-del-npv.md). **Queda una discrepancia declarada**: se indico verbalmente que la fecha es el cierre de 2026 y la aritmetica la fecha un ano antes, de modo que el modelo publica un 9,1 % por debajo de esa fecha. La consulta 1 del `CON` Rev. A hay que reemitirla como constancia. Lo anterior: `Resumen!89` rotula el indicador `NPV 2026 @ 10%` y la fila 92 lleva un `Costo Hundido (2017-25)`, de modo que el ejercicio 2025 esta fuera del alcance por decision. Pero **la aritmetica descuenta a 2025, no a 2026**: el ejercicio 2026 lleva `1/(1+r)^1` y no `1`, asi que el resultado es un NPV fechado en 2025 de los flujos de 2026 en adelante. Si la fecha buscada es 2026, todos los terminos van un ano sobredescontados y el indicador sale un 10 % por debajo. Es lo que hay que preguntarle a Finanzas | | 02/09/2026 | `indicadores.py` |
...
| 064 | `FC NZ!42` y `!49` | La TIR economica del caso 7 vale `-`: el `IFERROR` se disparo porque la funcion `IRR` de Excel no convergio sobre ese flujo | El libro no entrega TIR para este caso porque el flujo abre en positivo y su unica raiz es profundamente negativa. **Desde el 03/09/2026 el motor tampoco la entrega** y las dos herramientas coinciden: lo fija el [ADR 0011](../adr/0011-cuando-el-motor-entrega-tir.md). Contrastado sobre seis hojas, tres coinciden en no darla y tres dan la misma tasa | | 02/09/2026 | `indicadores.py` |
...
| 075 | `FC NZ!35`, `!41` y `!42` de los casos 1, 2 y 7 | Contrastado el flujo alimentando `flujos.flujo_del_caso` con las lineas del libro: **EBITDA ajustado, flujo operativo, flujo de inversiones y flujo economico coinciden ejercicio a ejercicio en los tres casos**. El flujo economico abre positivo y **baja de cero en los ejercicios de cierre** -siete desde 2048 en el caso 7, doce desde 2043 en el 1 y el 2-, pero su acumulado nunca llega a ser negativo | **Corregida el 02/09/2026 con el contraste del caso 2.** La redaccion anterior afirmaba que el flujo no cambia de signo y el libro dice lo contrario: hay cambio de signo y existe una raiz profundamente negativa, que el `IRR` de Excel no alcanza desde su semilla -de ahi el guion de su `IFERROR`- y que el motor dejo de entregar el 03/09/2026, por el [ADR 0011](../adr/0011-cuando-el-motor-entrega-tir.md). Es la regla `064` y no un rasgo del caso 2. **El payback si coincide**, por el acumulado y no por el signo: sin acumulado negativo no hay desembolso que recuperar y ninguna de las dos herramientas lo entrega. El unico indicador que difería era el NPV, por la regla `062`, **y desde el 04/09/2026 cierra con brecha 0,00**: el caso declara su ultimo ejercicio hundido y el ADR 0012 lo lleva al factor de descuento | Derivada del libro | 02/09/2026 | `flujos.py`, `indicadores.py` |
| 076 | `FC escenarios` y `Resumen!89` | La hoja tiene **diez bandas y solo la primera calcula**. La columna `H` lleva 910 formulas en sus filas de flujo -9 a 35- y es el caso activo; las nueve siguientes traen 945 celdas tecleadas cada una y ninguna formula, salvo dos que estan **enteramente vacias**, `Nazareth 2032` y `Nazareth 2038`. `Resumen!89` no lee la viva: lee la congelada | **El numero que el modelo publica no es el que el modelo calcula.** Las dos bandas rotuladas `Sin Proyecto` dan 4 862 348,7 la viva y 1 785 682,5 la pegada, un factor de 2,7. El archivo guarda resultados de una version anterior y no se ha vuelto a poblar. El motor reproduce **la viva** al centimo en los dos casos contrastados; la distancia con lo publicado no es del motor | | 02/09/2026 | |
| 077 | `Resumen!90` y `!91` | Los indicadores de decision del modelo son **incrementales contra `Sin Proyecto`**: `NPV Incremental (vs Sin Proyecto)` y `TIR Incremental` | Explica por que la TIR absoluta de `FC NZ!42` no informa nada aunque exista: el flujo de un caso con operacion en marcha abre positivo y solo baja de cero en los ejercicios de cierre, de modo que su unica raiz es profundamente negativa -el guion del libro es el `IFERROR` de un `IRR` que no converge, regla `064`-. **La que tiene sentido es la incremental**, sobre la diferencia entre el caso y el base, y hoy el motor no la calcula: entrega indicadores absolutos de una corrida. Es funcionalidad, no un defecto de fidelidad | | 02/09/2026 | `indicadores.py` |
| 078 | `FC escenarios!45`, bandas viva y congeladas | El **NPV incremental**, que es el indicador de decision del modelo, vale 1 536 271,6 k$ segun el calculo vivo y 338 045,0 segun las bandas publicadas: los casos 7 y 1 dan 6 398 620,3 y 4 862 348,7 vivos contra 2 123 727,5 y 1 785 682,5 congelados | **Un factor de 4,5 sobre la cifra con la que se decide invertir.** No es una diferencia de metodo ni de motor: es la distancia entre lo que el libro calcula hoy y lo que su hoja de resultados guarda de antes. Hay que resolverlo antes de la certificacion de fidelidad, porque el numero que el cliente reconoce es el publicado | | 02/09/2026 | |
| 079 | `InputsOpex`, bloque `Supuestos Pisco (Nazareth / SR Potencial)` | La refineria cobra de dos maneras. Los origenes de su bloque directo llevan los conceptos de la propia planta -fundicion, refineria, planta de subproductos, mantenimiento F&R-; el resto se cobra a una tarifa por tonelada fina, `Costo / tmf`, aplicada a lo que refina cada uno, con un subtotal por origen | Es un reparto de costo por origen, no un supuesto de precio. Que origenes van por el bloque directo **lo declara cada unidad** en su pestana de supuestos, con la bandera `Costo Directo en la Refineria`: en el libro son dos unidades concretas y en la plataforma no puede haber ninguna escrita en el codigo. El supuesto `Costo de Fundicion` ya se pedia en la plantilla y la ingesta lo descartaba; desde el 03/09/2026 llega al motor. **Se informa sin entrar al flujo**: sumarlo al cash cost cambia el NPV y esa parte queda pendiente de Finanzas | Project Manager | 03/09/2026 | `bloques.py`, solo presentacion |
| 080 | `Otros!33` y `!37` | La misma regalia se parte en dos bloques por el signo de `Impuestos!20`, la utilidad operativa: la 33 la lleva a `Otros Gastos/ Ingresos` con `IF(Impuestos!H20<0,-Impuestos!H25,0)` y la 37 a `Impuestos` con la condicion contraria. Las dos condiciones son estrictas, de modo que con utilidad **exactamente cero** no la recoge ninguna de las dos y la regalia desaparece de la hoja | Reclasificacion por resultado del ejercicio: en perdida manda la regalia minima sobre ventas y el libro la trata como gasto de operacion en vez de como tributo. Se reproducen las dos filas con su rotulo literal -`Regalias` con tilde y sin ella, que es lo unico que las distingue- y el hueco del cero se reporta sin corregirlo | | | `otros.py` |
| 081 | `Otros!43:55` frente a `corrida._bolsa_de_egresos` | La `Bolsa Egresos` del libro suma once conceptos y el motor sumaba doce: agregaba `Gestion Social`, que el libro no lleva en este bloque. El propio docstring de la funcion enumeraba los once del libro | **Corregida el 04/09/2026 por la regla `094`, y estaba mal.** La lectura se hizo sobre los rotulos de la bolsa y no sobre sus formulas: `Otros!48` se rotula `Donaciones` y es `InputsOpex!175`, que se llama `Gestión Social`. El libro **si** la lleva en su bolsa, bajo otro nombre, y la plataforma la habia sacado dejando en su lugar un concepto que el bloque de gastos del libro no tiene. La bolsa vuelve a llevarla y la prueba que fijaba esta regla pasa a afirmar lo contrario | Project Manager | 04/09/2026 | `otros.py`, `corrida.py` |
| 082 | `Otros!28:34` frente a `corrida.calcular` | El bloque `Otros Gastos/ Ingresos` del libro lleva donaciones, servidumbre, reguladores con el fondo de jubilacion, la fila tecleada `Otros` y la regalia de la regla `080`, y **no lleva la planilla**: el libro la teclea en `Otros!52`, dentro de la bolsa de egresos. El motor la suma a la linea de otros gastos del flujo operativo | **Resuelta el 04/09/2026 al leer `FC NZ!21`**, que es `Otros!34 - Otros!29` y no lleva la planilla. Era la segunda lectura: un doble cargo, porque `planilla = cash cost x tasa` es una fraccion de un costo que la fila `15` ya descuenta entero. Sale del flujo operativo y se queda donde el libro la usa, la bolsa de egresos, que alimenta las cuentas por pagar y el IGV de compras | Project Manager | 04/09/2026 | `corrida.py` |
| 083 | `Otros!17`, `!18` y `!19` frente a `supuestos.aplicar` | El libro calcula tres lineas de gasto de ventas -Sn refinado por `$/tmf`, concentrado de Cu y concentrado de Sn, las dos por `$/t conc`- y la plantilla pide las tres tarifas. La ingesta solo vuelca `Gasto de Ventas Conc. Sn` y descarta las otras dos, que no tienen campo en `DatosComunes` ni un solo lector | Mismo caso que el `Costo de Fundicion` de la regla `079`: el dato se pide, se lee y no llega al motor. Las dos tarifas necesitan campo y linea propia; hasta entonces la hoja muestra la linea consolidada y lo declara en su nota | | | `supuestos.py` |
...
| 090 | `FC NZ!41` frente a `!39` | El NPV **tiene dos candados y no uno**. Ademas del cero tecleado en el factor del primer ejercicio -la regla `062`-, la fila del NPV es `=SUM(I40:AQ40)` y arranca en la segunda columna | Los dos hacen lo mismo y son independientes: **corregir el cero no moveria el NPV**, porque la suma seguiria ignorando la primera columna. Es material para la consulta 1 a Finanzas, que sigue sin respuesta: si la fecha de valoracion es la que el rotulo dice, hay que tocar dos celdas | Derivada del libro | 04/09/2026 | |
| 091 | `FC NZ!21` | `Otros Gastos` es `Otros!34 - Otros!29`: el total del bloque de otros gastos menos su primera fila. Recoge la servidumbre, los reguladores con el fondo, la fila tecleada `Otros` y la regalia de los ejercicios en perdida | **Se construye restando posiciones y no sumando conceptos.** Insertar una fila en `Otros` entre la 29 y la 33 cambia el flujo sin que nada lo acuse. El motor la compone por conceptos, que es lo que el libro quiere decir | Derivada del libro | 04/09/2026 | `corrida.py` |
| 092 | `FC NZ!7` y `!9` | `Periodo pre-operativo` son 36 ceros tecleados sin formula, y `Ano cierre` se calcula comparando la depreciacion contra un umbral `20000` incrustado en su propia formula. **Ninguna celda del libro lee ninguna de las dos** | Dos filas muertas dentro de la hoja del caso. No se reproducen: la primera no aporta nada y la segunda obligaria a implementar una constante magica para una fila que nadie consume | Derivada del libro | 04/09/2026 | `flujos.py` |
| 093 | `FC NZ!42` frente a `!41` | La TIR se calcula sobre `H:AQ` y el NPV sobre `I:AQ`: **dos criterios de horizonte en filas adyacentes** | La regla `015` registro esta asimetria entre `FC NZ` y `FC escenarios` y concluyo que en `FC NZ` los dos indicadores cubrian el mismo rango. No es asi. El motor los calcula sobre la misma serie, que sigue siendo la decision correcta, y la asimetria del libro se reporta | Derivada del libro | 04/09/2026 | `indicadores.py` |
| 094 | `FC NZ!20`, `Otros!29` y `!48` frente a `InputsOpex!175` | Las tres filas leen la misma celda. `FC NZ!20` la rotula `Gestión Social`, las dos de `Otros` la rotulan `Donaciones` y en su propia hoja se llama `Gestión Social`. **El bloque `InputsOpex!173:182` no tiene ninguna fila de donaciones** | Un concepto con dos nombres, no dos conceptos. La regla `055` lo dedujo del rotulo de `Otros!29` sin leer su formula, y desde entonces la plataforma pedia las dos cosas en su plantilla y cargaba las dos al flujo. **El rotulo se conserva donde el libro lo escribe -es el que el cliente reconoce- y el calculo pasa a ser el del libro**: `Donaciones` sale de la plantilla y las filas que lo llevan leen la gestion social. **Corrige tambien la `081`** | Project Manager | 04/09/2026 | `opex.py`, `corrida.py` |
| 095 | `FC NZ!35` | El flujo economico es `27 + 33 - 25`: el flujo operativo descuenta los intereses y esta linea los devuelve | **Es un flujo anterior al financiamiento**, de modo que el resultado no depende de como se financie el proyecto. El motor los dejaba dentro y no se veia porque los intereses valen cero en los siete casos del arnes y en el caso 7 del libro. Se alinea al modelo | Project Manager | 04/09/2026 | `flujos.py` |
| 096 | `InputsProd!90:99` | De los cinco origenes de Pisco, **tres llegan por formula desde su propia produccion** -Nazareth `=JN43`, San Rafael Potencial `=JN67`, Santo Domingo `=JN87`, con su ley en `JN85`- y **dos se importan del libro LOM externo**, el mismo `[8]` de la regla `054`: San Rafael desde `[8]InputsProd!PJ43` y B2 desde `!PJ45`, filas distintas de las que traen su produccion. Por eso su alimentacion no coincide con lo que producen: San Rafael difiere en cuatro de los diez primeros ejercicios -hasta un 3,5 %- y B2 en los cuatro que tiene activos, con una ley que en uno se aparta 5,1 puntos | **El libro calcula la alimentacion de sus proyectos y la importa para sus dos unidades en marcha**, que tienen su propio libro de plan de mina con calendario de embarque y acopio. La plataforma evalua proyectos y la calcula para todos los origenes, que es lo que corresponde y lo que la regla de oro exige: pedirla como dato dejaria al usuario tecleando lo que el modelo deriva para cualquier proyecto nuevo. **La diferencia en San Rafael y B2 es prevalencia del LOM**, de la familia de la `054` y de la desviacion `D-01`, y no es reproducible sin ese libro externo. Se reproduce el criterio del libro para lo que la plataforma evalua, y la diferencia con esas dos unidades se reporta | Project Manager | 04/09/2026 | `refineria.py` |
| 097 | `InputsProd!31:37` | El bloque de B2 tiene forma propia -sin preconcentracion ni cash cost separado- y **queda fuera del contraste de produccion**, que recorre San Rafael, San Rafael Potencial y Santo Domingo | Un bloque sin contraste es un bloque cuyas discrepancias no se ven, y las de B2 existen: su entrega a Pisco difiere de su produccion en los cuatro ejercicios que tiene activos. El arnes tiene que cubrir su forma antes de dar la produccion por contrastada | Derivada del libro | 04/09/2026 | |
| 098 | `InputsOpex!75` frente a `!131`, `!142`, `!155` y `!166` | La fila `Planillas` del bloque de cash cost de la refineria **es un dato importado** -`[8]InputsOpex!PJ31`- y el catalogo de la plataforma la excluia con el argumento de que se deriva del cash cost por la tasa de los supuestos | **Ese argumento es de otra fila.** La que se deriva es la `Planilla` del bloque de gastos, que es la regla `026`; la de la refineria es un concepto de costo con su propio origen externo, y los dos rotulos se parecen lo suficiente para que la exclusion se aplicara a la que no era. Sin ella salian cortos el total de la refineria -`InputsOpex!76`-, el consolidado -`!86`- y el `Total Cash Cost` -`!88`- que `Otros!13` lleva al flujo. **Entra al catalogo**, al final del bloque de la refineria para no desplazar lo que la plantilla ya emitia | Project Manager | 04/09/2026 | `opex.py` |
...
la fila 58, la columna de total que excluye el primer ejercicio, la bandera de produccion sin nombre
de unidad y el reparto de la regalia por el signo del resultado. Una la resolvio el Project Manager
alineandose al modelo -la `081`, la gestion social que el motor sumaba a una bolsa de egresos donde
el libro no la lleva-. Y **dos cambian cifras y se consultan antes de tocar el motor**: la `082`, la
planilla que el libro teclea en la bolsa y el motor suma tambien al flujo operativo, y la `083`, dos
tarifas de gasto de ventas que la plantilla pide y la ingesta descarta. Del hueco de la `080` -con
utilidad operativa exactamente cero la regalia no cae en ninguno de los dos bloques- se informa sin
corregirlo.
...
**Seis mas salieron el 04/09/2026 de dar a la hoja `FC NZ` la forma del libro**, de la `090` a la
`095`, y con ellas se cerro la lectura fila a fila de las cinco hojas de calculo por ano. Cuatro son
derivables de la formula y se implementan citandolas: los dos candados del NPV, la fila que se
construye restando posiciones, las dos filas muertas y los dos criterios de horizonte. Las otras dos
las decidio el Project Manager alineandose al modelo: la `094`, que retira `Donaciones` de la
plantilla porque el bloque de gastos del libro no la tiene, y la `095`, que devuelve los intereses
al flujo economico.

**Esa misma lectura cerro la `082` y corrigio la `081`.** La `082` sospechaba un doble cargo de la
planilla y la formula de `FC NZ!21` lo confirmo. La `081` se habia decidido dos dias antes leyendo
el rotulo de una fila de la bolsa en vez de su formula, y era el mismo rotulo enganoso que causo la
`094`: conviene recordar que **en este libro el nombre de una fila no basta para saber que lleva**.

**Y dos mas el 04/09/2026, al mirar por que el contraste del flujo cerraba y el de la produccion
no.** La `096` es la de fondo, y confirma el criterio del motor en vez de cambiarlo: el libro
**calcula** la alimentacion de Pisco para sus tres proyectos y solo la importa para sus dos
unidades en marcha, que tienen libro de plan de mina propio. La plataforma la calcula para
todos, que es lo que corresponde a evaluar un proyecto; lo que no puede es reproducir el
calendario de embarque de un libro que no tiene. La `097` anota
que B2 no se estaba contrastando, de modo que sus discrepancias no aparecian en ningun informe.

**Conviene recordar por que hizo falta buscarlas.** Los contrastes del flujo y de los indicadores
alimentan al motor con las lineas del propio libro para aislar el bloque; su brecha cero dice que la
aritmetica del flujo coincide y **no dice nada sobre la cadena que lo produce**. Leerla como si lo
dijera fue lo que dejo estas dos sin ver.

La `098` sale del mismo trabajo y es de la familia de la `094`: **dos filas del libro que se llaman
casi igual y hacen cosas distintas**, y una decision tomada sobre el rotulo en vez de sobre la
formula. Van tres en dos dias -la gestion social que el libro rotula `Donaciones`, la planilla de la
refineria frente a la del bloque de gastos, y la regalia repartida entre dos rotulos que solo
difieren en la tilde-, de modo que conviene tomarlo como norma y no como anecdota: **en este libro
el nombre de una fila no basta para saber que lleva, y hay que seguir su cadena hasta el final.**
